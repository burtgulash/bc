...
pokud zajistíme synchronizaci vláken při přístupu k hodnotám betweeness.
Narozdíl od closeness, kde výpočet pro jeden uzel ovlivňuje hodnotu closeness
pouze pro tento uzel, u betweeness výpočet vycházející z jednoho uzlu ovlivňuje
hodnotu betweeness potencionálně i pro všechny ostatní uzly.
...
\begin{algorithm}[H]
	...
	\For{$s \in V$}
		...
		\While{$S \ne \varnothing$}
			\State $w \gets pop(S)$
			\For{$v \in P[w]$}
				\State $\delta[v] + \frac{\sigma[v]}{\sigma[w]}(1 + \delta[w])$
			\EndFor
			\If{$w \ne s$}
...
betweeness a BFS. V tomto případě do prioritní fronty prvotně zahrneme všechny
...
\input{../dblp/tabels/approximatedbetweenessnover4}
\input{../dblp/tabels/betweenessParallel}
\input{../dblp/tabels/wBetweeness}
\input{../dblp/tabels/hindex}
\end{footnotesize}

\section{CiteSeer}
\begin{footnotesize}
\input{../citeseer/tabels/outdegree}
\input{../citeseer/tabels/indegree}
\input{../citeseer/tabels/wOutdegree}
\input{../citeseer/tabels/wIndegree}
\input{../citeseer/tabels/pagerank}
\input{../citeseer/tabels/inCloseness}
\input{../citeseer/tabels/wCloseness}
% \input{../citeseer/tabels/approximatedBetweeness_Nover4}
\input{../citeseer/tabels/betweenessParallel}
\input{../citeseer/tabels/wBetweeness}
\input{../citeseer/tabels/hindex}
\end{footnotesize}
