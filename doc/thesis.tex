...
\chapter*{Abstract}
\thispagestyle{empty}
Elements of complex networks without any prior assignment of importance can
still be ranked by importance determined by connections with other elements.

We discuss the notion of importance of a node in network and explore various
existing methods for measuring node's importance (centrality measures and
...
highlighted). We also discuss difference between in and outedges and difference
...
Dijkstrův algoritmus~\citep{dijkstra1959} řeší \uv{single source shortest path
problém} pomocí greedy (žravé) strategie. Časová náročnost běžné implementace
...
sobě a co naznačuje, že skutečně měříme významnost.
